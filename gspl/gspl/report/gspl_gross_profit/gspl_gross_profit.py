...
class GrossProfitGenerator(object):
	...
	def process(self):
		...
		if self.filters.group_by == "Customer":
			customer_wise_gle = self.get_customer_wise_gle()

		if self.filters.group_by == "Brand":
			brand_wise_i = self.get_stock_interest("Brand")

		if self.filters.group_by == "Item Group":
			group_wise_i = self.get_stock_interest("Item Group")
		...
		for row in reversed(self.si_list):
			...
			if self.filters.group_by == "Customer":
				interest = customer_wise_gle.get(row.customer, 0)
				row.interest = interest

			if self.filters.group_by == "Brand":
				interest = brand_wise_i.get(row.brand, 0)
				row.interest = interest

			if self.filters.group_by == "Item Group":
				interest = group_wise_i.get(row.item_group, 0)
				row.interest = interest
	...
	def get_stock_interest(self,group_by):
		brand_wise_i = {}
		group_wise_i = {}
		...
		query = (
			...
			.where(sle.posting_date <= getdate(self.filters.get("to_date")))
		)
		...
		for sle in sle_entries:
			brand = sle.brand
			group = sle.item_group
			if not brand:
				continue

			# Convert posting_date to date object
			posting_date = getdate(sle.posting_date)
			...
			interest = (
				(sle.stock_value_difference) * (date_diff(to_date,posting_date) + 1)
				* 9 / 36500
			)

			brand_wise_i.setdefault(brand, 0)
			brand_wise_i[brand] += interest

			group_wise_i.setdefault(group, 0)
			group_wise_i[group] += interest
		if group_by == "Brand":
			return brand_wise_i
		else:
			return group_wise_i		

	def get_customer_wise_gle(self):
		customer_wise_gle = {}

		gl_entries = frappe.db.get_all(
			"GL Entry",
			filters={"party_type": "Customer", "docstatus": 1},
			fields=["name", "posting_date", "account", "debit", "credit", "party"]
		)
		...
		for gle in gl_entries:
			...
			interest = (
				(gle.debit - gle.credit) * (date_diff(to_date,posting_date) + 1)
				* 9 / 36500
			)

			customer_wise_gle.setdefault(party, 0)
			customer_wise_gle[party] += interest

		return customer_wise_gle
