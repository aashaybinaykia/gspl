...
from frappe.utils import cint, flt, getdate, logger
...
logger = frappe.logger("gspl_consolidated_financial_statements", allow_site=True, file_count=10)
...
def get_data(
	...
	change_values(accounts_by_name, filters)
...
def change_values(accounts_by_name,filters):
    # Define the target account and company
	target_company = 'Gautam Syntex Pvt Ltd'

	opening_year = filters.get("from_fiscal_year")
	closing_year = filters.get("to_fiscal_year")
	...
	closing_balance = frappe.db.get_value(
        'Closing Stock',
        {'fiscal_year': closing_year},
        'closing_stock'
    )
	closing_balance = closing_balance or 0
	closing_year_closing = frappe.db.get_value("Fiscal Year", {"name": closing_year}, "closing_stock") or 0
	opening_year_closing = frappe.db.get_value("Fiscal Year", {"name": opening_year}, "opening_stock") or 0
	delta = 65543325.1



	for account_name, account_data in accounts_by_name.items():
		# Check if the current account is the target account and the target company
		if (
			account_name == 'Stock In Hand'
			and account_data['company'] == target_company
		):

			account_data['opening_balance'] = opening_year_closing

			# if closing_balance > 0:
			# 	account_data[target_company] = closing_balance
			# else: 
			account_data[target_company] = closing_year_closing
			# Update company_wise_opening_bal for the target company
			account_data['company_wise_opening_bal'][target_company] = opening_year_closing
		...
		if (
			...
			total_purchase = get_total_purchase(filters)
			logger.info("total_purchases %s", total_purchase)

			account_data[target_company] = total_purchase
			account_data['account_name'] = 'Total Purchase'
			account_data['name'] = 'Stock In Hand - GSPL'
		...
		if (
			...
			# Update current value in Gautam Syntex Pvt Ltd to 1000
			# if closing_balance == 0:
			# 	change = get_stock_in_hand_difference (filters)
			# else:
			change = opening_year_closing - closing_year_closing
			account_data[target_company] = change
			account_data['account_name'] = 'Change in Stock'
			account_data['name'] = 'Stock In Hand - GSPL'
		...
		if (
			...
			account_data['opening_balance'] += delta

			if closing_balance < 0:
				account_data[target_company] = closing_balance
			else: 
				account_data[target_company] += delta
			# Update company_wise_opening_bal for the target company
			account_data['company_wise_opening_bal'][target_company] += delta

    

    # Iterate through the accounts_by_name dictionary
...
def get_total_purchase (filters):
	...
	query = (
		...
		.where((gle.docstatus < 2) & (gle.is_cancelled == 0) & (gle.voucher_type.isin(["Purchase Invoice"]) & (gle.posting_date >= start_date) & (gle.posting_date <= end_date) & (gle.account.isin(["Stock Adjustment - GSPL", "Cost of Goods Sold - GSPL", "Stock In Hand - GSPL"]))))
		
	)
	purchases = query.run(as_dict=True)

	logger.info("purchases %s", purchases)

	
	total_debit = sum(row["debit"] for row in purchases)
	total_credit = sum(row["credit"] for row in purchases)

	# Calculate the difference (debit - credit)
	balance = total_debit - total_credit
	return balance

def get_stock_in_hand_difference (filters):
	fiscal_year = get_fiscal_year_data(filters.get("from_fiscal_year"), filters.get("to_fiscal_year"))
	...
	query = (
		...
		.where((gle.docstatus < 2) & (gle.is_cancelled == 0) & (gle.posting_date >= start_date) & (gle.posting_date <= end_date) & (gle.account.isin(["Stock In Hand - GSPL"])))
		
	)
	stock = query.run(as_dict=True)


	
	total_debit = sum(row["debit"] for row in stock)
	total_credit = sum(row["credit"] for row in stock)
