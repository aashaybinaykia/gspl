...
class Analytics(object):
	...
	def get_sales_transactions_based_on_order_type(self):
		...
		extra_conditions = ""
		tuple_conditions = [self.filters.company, self.filters.from_date, self.filters.to_date]
		has_item_filters, items = self.get_filtered_items()
		if has_item_filters:
			extra_conditions += " and i.item_code in (%s)" % (",".join(["%s"] * len(items)))
			tuple_conditions.extend(items)
		...
		self.entries = frappe.db.sql(
			...
			and ifnull(s.order_type, '') != '' order by s.order_type {extra_conditions}
		""".format(
				date_field=self.date_field, value_field=value_field, doctype=self.filters.doc_type, extra_conditions=extra_conditions
			),
	...
	def get_sales_transactions_based_on_customers_or_suppliers(self):
		...
		has_item_filters, items = self.get_filtered_items()
		if has_item_filters:
			child_doctype = self.filters.doc_type + " Item"
			...
			child_items = list(map(lambda item: item['parent'], child_items))
			extra_filters['name'] = ["in", child_items]
	...
	def get_sales_transactions_based_on_items(self):
		...
		self.entries = frappe.db.sql(
			...
			and s.{date_field} between %s and %s {extra_conditions}
		""".format(
				date_field=self.date_field, value_field=value_field, doctype=self.filters.doc_type, extra_conditions=extra_conditions
			),
			# (self.filters.company, self.filters.from_date, self.filters.to_date),
	...
	def get_filtered_items(self):
		...
		if self.filters.brand:
			filters['brand'] = self.filters.brand
		if self.filters.item_template:
			filters['variant_of'] = self.filters.item_template

		if filters:
			items = frappe.get_all(
				"Item", filters=filters, fields=['name',]
			)
			items = list(map(lambda item: item['name'], items))
			has_filters = True

		return has_filters, items
